...
import { FindVente, PrixUnitaireSystem, UpdateCaisses, UpdateDetailPanier, UpdatePaiement, UpdateProduit, UpdateVente } from "@/prisma/utils";
    ...
    try {
        ...
        if (detailPanier.qtte > form.qtte) {
            if (detailPanier.id == parseInt(detailId) && detailPanier.produitId == form.produitId) {
                ...
                let nouveauPrixTotalTTC = montantRetrait * 0.16;

                if (vente) {
                    const updateDetailPanier = await UpdateDetailPanier(detailPanier.id, form);
                    UpdateProduit(updateDetailPanier.produitId, 'inc', differenceRecue);
                    UpdateVente(vente.id, 'dec', montantRetrait, nouveauPrixTotalTTC);
                    const paiement = await UpdatePaiement(vente.paiementId, 'dec', montantRetrait);
        ...
        if (detailPanier.qtte < form.qtte) { // Decrementation
            if (detailPanier.id == parseInt(detailId) && detailPanier.produitId == form.produitId) {
                ...
                if (vente) {
                    ...
                    produit ? await UpdateProduit(produit?.id, 'dec', differenceRemue) : '';
